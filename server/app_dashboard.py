# file: server/app_dashboard.py
"""
ProFit AI Studio - Dashboard Principale
Pagina di atterraggio con KPI, sessioni prossime e azioni rapide.
"""
...
# ════════════════════════════════════════════════════════════
# KPI ROW - Dati reali (render_metric_box unificato)
# ════════════════════════════════════════════════════════════
...
with col1:
    render_metric_box(
        "Clienti Attivi", str(n_clienti), "portfolio attuale", "",
        value_color="var(--primary)", trend_class="neutral"
    )

with col2:
    render_metric_box(
        "Sessioni Oggi", str(n_eventi_oggi),
        "in programma" if n_eventi_oggi > 0 else "nessuna sessione", "",
...
with col3:
    render_metric_box(
        "Entrate Mese", format_currency(entrate_mese, decimals=0),
        oggi.strftime('%B %Y').capitalize(), "",
        value_color="var(--accent)", trend_class="neutral"
    )

with col4:
    n_stale = len(sessioni_stale)
    render_metric_box(
        "Sessioni Scadute", str(n_stale),
        "da gestire!" if n_stale > 0 else "tutto in ordine", "",
        value_color="var(--danger)" if n_stale > 0 else "var(--secondary)",
        trend_class="negative" if n_stale > 0 else "positive"
    )
