...
- `UPG-2026-03-10-16`: chiuso un audit documentale mirato sui file autorevoli post hardening. `CLAUDE.md`, `frontend/CLAUDE.md`, `TAILSCALE_FUNNEL_SETUP` e la checklist release parlano ora la stessa lingua del prodotto reale: `src/proxy.ts` al posto del vecchio `middleware.ts`, nuovi artefatti di supporto (`Stato installazione`, `Snapshot diagnostico`, `SUPPORT_RUNBOOK.md`) esplicitati, e piano operativo `UPG-2026-03-10-09` chiarito con un `Progress Update` che separa backlog storico e stato corrente. Decisione chiave: la documentazione non deve solo essere vera, deve anche evitare che un documento "storico" sembri smentire il prodotto di oggi.
- `UPG-2026-03-10-15`: formalizzato il runbook operativo unico di supporto locale. Ora installazione, licenza, backup/restore e recovery post-update non dipendono piu da memoria sparsa tra documenti o terminale: il punto di partenza canonico diventa `docs/SUPPORT_RUNBOOK.md`, costruito sopra `Stato installazione`, `Snapshot diagnostico`, backup v2 e pagina `/licenza`. Decisione chiave: per distinguersi come software locale affidabile non basta essere robusti; bisogna essere anche supportabili in modo ripetibile.
