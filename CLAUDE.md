...
| `--reload` su backend PROD | Backend prod con `--reload` → ogni modifica .py (anche da Codex/Claude) riavvia il server → API call in volo falliscono → errori intermittenti su agenda e CRUD | MAI `--reload` in prod. `restart-backend.sh` gia' gestisce: `prod` senza reload, `dev` con reload |
...
| Proxy Next.js intercetta prima dei rewrite | `fetch('/api/public/*')` senza cookie JWT viene bloccato dal proxy prima che il rewrite lo proxyi al backend → `307 /login` → client riceve HTML invece di JSON (404 o parse error) | Separare `PUBLIC_ROUTES` (accessibili senza auth) da `AUTH_ONLY_PAGES` (solo le pagine che redirectano utenti autenticati). Aggiungere `/api/public` a `PUBLIC_ROUTES` esplicitamente |
| Link kiosk generato da localhost inutilizzabile su altri device | `window.location.origin = "http://localhost:3000"` → link non raggiungibile da smartphone su rete diversa | Configurare `PUBLIC_BASE_URL=https://<nome>.ts.net` in `data/.env` → il backend genera URL assoluti con dominio Funnel. Fallback: warning amber se localhost e PUBLIC_BASE_URL non configurato |
...
| `DialogHeader` fuori da `DialogContent` | `<Dialog><DialogHeader>...<DialogContent>...` — il `DialogHeader` e' figlio diretto di `Dialog.Root`, renderizzato sempre. Il `DialogContent` (nel Portal) non viene renderizzato se `open=false`. Server/client possono divergere sull'ordine dei nodi → hydration mismatch | Spostare `DialogHeader` DENTRO `DialogContent`. Quando `open=false`, il Portal non renderizza nulla → zero mismatch. **Regola**: in Radix Dialog, TUTTO il contenuto visuale deve stare dentro `DialogContent` |
