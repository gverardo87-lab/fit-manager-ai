...
File chiave: `lib/workout-templates.ts` (template + `getSectionForCategory` + `getSmartDefaults`), `lib/derived-metrics.ts` (PATTERN_TO_1RM), `lib/muscle-map-utils.ts` (`MUSCLE_SLUG_MAP`, `SMART_TO_SLUG_MAP`, `buildBodyData`), `lib/export-workout.ts` (`addExerciseCard`, `addBlockExerciseCard`, `BLOCK_TYPE_HEADER_COLORS`, `BLOCK_TYPE_COLORS`), `components/workouts/SessionCard.tsx` (3 sezioni DnD, volume, overflow menu), `components/workouts/BlockCard.tsx` (blocchi strutturati, usa `BLOCK_EXERCISE_CONFIG`), `components/workouts/SortableExerciseRow.tsx` (grid 8-col, `BLOCK_EXERCISE_CONFIG` export, % 1RM badge, espansione unificata), `components/workouts/WorkoutPreview.tsx` (`BlockPreview` con `BLOCK_TYPE_PREVIEW_CONFIG`), `components/workouts/ExerciseDetailPanel.tsx` (dettaglio inline), `components/workouts/MuscleMapPanel.tsx` (silhouette 3-colori status-aware).
